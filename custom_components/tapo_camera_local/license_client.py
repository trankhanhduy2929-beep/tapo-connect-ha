...
def load_policy() -> LicensePolicy:
    try:
        data = json.loads(Path(__file__).with_name("license_policy.json").read_text())
        policy = LicensePolicy(**data)
        if type(policy.enabled) is not bool or type(policy.grandfather_existing) is not bool:
